...
impl worker::Worker<BootlogParserWorkerArgs> for BootlogParserWorker {
    async fn perform(&self, args: BootlogParserWorkerArgs) -> worker::Result<()> {
        ...
        let _device =
            match _entities::devices::Model::find_device(&self.ctx.db, &args.dongle_id).await {
                Ok(device) => device,
                Err(_e) => {
                    tracing::info!(
                        "Recieved file from an unregistered device. Do something: {}",
                        &args.dongle_id
                    );
                    return Ok(());
                }
        ...
        match _entities::bootlogs::Model::add_bootlog(
            ...
            &args.dongle_id,
            &format!(
                "{api_endpoint}/connectdata/bootlog/{}",
            ...
            &format!(
                ...
                common::mkv_helpers::get_mkv_file_url(&format!(
                    "{}_{}",
                    &args.dongle_id,
                    &args
                        .file_name
...
async fn parse_bootlog(decompressed_data: Vec<u8>) -> worker::Result<ParsedLog> {
    ...
    {
        let event: log_capnp::event::Reader = message_reader
            .get_root::<log_capnp::event::Reader>()
            .map_err(Box::from)?;
        ...
        match event.which().map_err(Box::from)? {
            log_capnp::event::Boot(log) => {
                ...
                if bootlog.has_launch_log() {
                    writeln!(writer, "{:#?}", bootlog.get_launch_log().unwrap())
                        .map_err(Box::from)?;
                }
                ...
                if bootlog.has_commands() {
                    ...
                    let re_date =
                        regex::Regex::new(r"end at \w+ (\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) \w+.")
                            .unwrap();
...
async fn upload_data(client: &Client, url: &String, body: Vec<u8>) -> worker::Result<()> {
    let response = client.put(url).body(body).send().await.map_err(Box::from)?;
