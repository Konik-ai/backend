...
pub async fn asset_download(
    ...
    match res {
        Ok(response) => {
            ...
            if has_range && status == StatusCode::OK {
                // override the file servers response
            ...
            for (key, value) in response.headers().iter() {
                ...
                if is_video
                    && (key == hyper::header::CONTENT_TYPE
                        || key == hyper::header::CONTENT_DISPOSITION)
                {
                    continue;
                }
            ...
            response_builder =
                response_builder.header(hyper::header::CACHE_CONTROL, "public, max-age=31536000");
...
pub async fn ensure_user_is_owner(
    ...
    if let Some(user_model) = &auth.user_model {
        if !user_model.superuser {
            DM::ensure_user_device(&db, user_model.id, &dongle_id)
                .await
...
pub async fn auth_file_download(
    ...
    if !public && !owner && !superuser {
        return Err((
            ...
            "You are not the owner and the route is not public",
        ));
    }
...
pub async fn depreciated_auth_file_download(
    auth: crate::middleware::auth::MyJWT,
    Path((_file, dongle_id, route_name, segment, file)): Path<(
        String,
        String,
        String,
        String,
...
async fn delete_route(
    ...
    for key_value in keys {
        ...
        let _ =
            client.delete(&file_url).send().await.map_err(|e| {
...
async fn get_cloudlog_cache(
    ...
    if let Some(user_model) = auth.user_model {
        ...
        if !user_model.superuser {
            if let Some(owner_id) = device_model.owner_id {
                ...
                return Err((
                    StatusCode::UNAUTHORIZED,
                    "Only registered devices can upload",
                ));
            }
        }
    } else {
    ...
    if let Some(device_logs) = cloudlog_cache.get(&dongle_id) {
        ...
        Err((
            StatusCode::NOT_FOUND,
            "No cloudlog data found for this dongle",
        ))
    }
}
...
pub async fn get_all_cloudlogs(
    ...
    let is_frogai = auth
        .user_model
        .as_ref()
        .map_or(false, |u| u.identity == frogai_id);
    if let Some(user_model) = &auth.user_model {
        if is_frogai {
            if let Some(ref filter_branch) = query.branch {
                if !filter_branch.starts_with("FrogPilot") {
                    return Err((
                        StatusCode::UNAUTHORIZED,
                        "You are not authorized to access all cloudlogs",
                    ));
                }
            } else {
            ...
            for (module, logs) in modules.iter() {
                ...
                for log in logs.iter() {
                    ...
                    if let Some(created) = log.get("created").and_then(|v| v.as_f64()) {
                        ...
                        let dt = Utc
                            .timestamp_opt(created as i64, (created.fract() * 1e9) as u32)
                            .single()
                    ...
                    if let Some(ctx) = log.get("ctx") {
                        if let Some(ref ctx_branch) = query.ctx_branch {
                            if ctx.get("branch").and_then(|v| v.as_str()) != Some(ctx_branch) {
                                continue;
                            }
                        }
                        if let Some(ref ctx_commit) = query.ctx_commit {
                            if ctx.get("commit").and_then(|v| v.as_str()) != Some(ctx_commit) {
                                continue;
                            }
                        }
                        if let Some(ref ctx_device) = query.ctx_device {
                            if ctx.get("device").and_then(|v| v.as_str()) != Some(ctx_device) {
                                continue;
                            }
                        }
                        if let Some(ctx_dirty) = query.ctx_dirty {
                            if ctx.get("dirty").and_then(|v| v.as_bool()) != Some(ctx_dirty) {
                                continue;
                            }
                        }
                        if let Some(ref ctx_dongle_id) = query.ctx_dongle_id {
                            if ctx.get("dongle_id").and_then(|v| v.as_str()) != Some(ctx_dongle_id)
                            {
                                continue;
                            }
                        }
                        if let Some(ref ctx_origin) = query.ctx_origin {
                            if ctx.get("origin").and_then(|v| v.as_str()) != Some(ctx_origin) {
                                continue;
                            }
                        }
                        if let Some(ref ctx_version) = query.ctx_version {
                            if ctx.get("version").and_then(|v| v.as_str()) != Some(ctx_version) {
                                continue;
                            }
                        }
                    } else {
                        // If ctx filter is set but log has no ctx, skip
                        if query.ctx_branch.is_some()
                            || query.ctx_commit.is_some()
                            || query.ctx_device.is_some()
                            || query.ctx_dirty.is_some()
                            || query.ctx_dongle_id.is_some()
                            || query.ctx_origin.is_some()
                            || query.ctx_version.is_some()
                        {
                            continue;
                        }
                    }
...
pub async fn api_useradmin_log_keys(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|_| (StatusCode::BAD_GATEWAY, "Failed to fetch log file"))?;
    if !resp.status().is_success() {
    ...
    let data = resp
        .text()
        .await
        .map_err(|_| (StatusCode::BAD_GATEWAY, "Failed to read log file"))?;
    let json: JsonValue =
        serde_json::from_str(&data).map_err(|_| (StatusCode::BAD_REQUEST, "Invalid JSON"))?;
    let obj = json
        .as_object()
        .ok_or((StatusCode::BAD_REQUEST, "Expected JSON object at top level"))?;
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("connectdata")
        .add(
            "/:dongle_id/:timestamp/:segment/coords.json",
            get(coords_download),
        )
        .add(
            "/:dongle_id/:timestamp/:segment/events.json",
            get(events_download),
        )
        .add(
            "/:dongle_id/:timestamp/:segment/sprite.jpg",
            get(sprite_download),
        )
        .add(
            "/:dongle_id/:timestamp/:segment/:file",
            get(auth_file_download),
        )
        .add(
            "/:filetype/:dongle_id/:timestamp/:segment/:file",
            get(depreciated_auth_file_download),
        )
