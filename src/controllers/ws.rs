...
use crate::models::{_entities, device_msg_queues::DMQM, devices::DM};
...
impl IntoResponse for Error {
    fn into_response(self) -> axum::response::Response {
        match self {
            ...
            Error::Timeout => {
                (axum::http::StatusCode::REQUEST_TIMEOUT, "Timed out").into_response()
            }
...
pub struct ConnectionManager {
    ...
    pub cloudlog_cache:
        RwLock<HashMap<String, HashMap<String, HashMap<String, Vec<serde_json::Value>>>>>,
}
...
async fn forward_command_to_device(
    ...
    let device_sender = {
        ...
        let device_conn = device_connections
            .get(endpoint_dongle_id)
...
async fn handle_socket(
    ...
    if is_device {
        ...
        let mut devices: tokio::sync::MutexGuard<HashMap<String, DeviceConnection>> =
            manager.devices.lock().await;
        tracing::info!("Adding device to manager: {}", endpoint_dongle_id);
        devices.insert(
            endpoint_dongle_id.clone(),
            DeviceConnection {
    ...
    while let Some(message_result) = receiver.next().await {
        ...
        match message {
            ...
            Message::Pong(_) => {
                ...
                let mut device_active_model = device.into_active_model();
                device_active_model.last_athena_ping = ActiveValue::Set(
                    SystemTime::now()
            ...
            Message::Close(_) => {
                tracing::debug!(
                    "{} WebSocket Closed {endpoint_dongle_id}",
                    if is_device { "Device" } else { "Client" }
                );
                break;
            }
            Message::Text(text) => {
                                ...
                                "forwardLogs" => {
                                    if let Some(params) = req.params {
                                            ...
                                            params.get("logs").and_then(|v| v.as_str())
                                        {
                                            ...
                                            {
                                                let mut cloudlog_cache =
                                                    manager.cloudlog_cache.write().await;
                                                ...
                                                for log in parsed_logs {
                                                    ...
                                                    let branch_map = device_logs
                                                        .entry(branch)
                                                        .or_insert_with(HashMap::new);
                                                    // Get or create the vector for the module.
                                                    let module_logs = branch_map
                                                        .entry(module)
            ...
            Message::Binary(_bin) => {
                // lots of cloudlogs coming in here. Maybe send it to the web interface client for real-time rollout monitoring
...
async fn handle_device_ws(
    ...
    } else if auth.claims.identity != endpoint_dongle_id {
        // if a device is trying to connect to another device
        tracing::error!(
            "Someone is trying to make illegal access: from {} to {endpoint_dongle_id}",
...
pub async fn send_ping_to_all_devices(manager: Arc<ConnectionManager>, db: &DatabaseConnection) {
    let device_senders: Vec<(String, mpsc::Sender<Message>)> = {
        ...
        devices
            .iter()
            .map(|(dongle_id, device_connection)| {
                (dongle_id.clone(), device_connection.sender.clone())
            })
            .collect()
    };
    ...
    for (dongle_id, device_sender) in device_senders {
        if let Ok(Some(latest_msg)) = DMQM::find_latest_msg(db, &dongle_id).await {
            if let Err(e) = device_sender
                .send(Message::Text(latest_msg.json_rpc_request.to_string()))
                .await
            {
                tracing::error!("Failed to send jsonrpc msg to device {}: {}", dongle_id, e);
            }
            if _entities::device_msg_queues::Entity::delete_by_id(latest_msg.id)
                .exec(db)
                .await
                .is_err()
            {
...
pub async fn send_reset(
    // called from crate::middleware::auth
    _ctx: &AppContext,
    socket: WebSocket,
) {
